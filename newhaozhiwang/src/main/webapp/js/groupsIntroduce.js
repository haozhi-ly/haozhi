$(function(){
	...
	$.post("groups/showsearch",{"_method":"POST","groupname":groupname,"userid":userid},function(gdata){
		...
		$.post("groups/user",{"_method":"POST","groupname":groupname,"userid":userid},function(data){
			...
			var listStr="";
			if(data>0){
				listStr+=gdata.groupname+'<a id="exit-btn" class="btn btn-default btn-sm mlm" href="javaScript:void(0);">退出小组</a>';
				$("#gname").html(listStr);
				var gexit='<span class="text-muted fsn ">你已经是小组成员，<a id="exit-btn" class="text-muted" href="groups/exitgroup?userid=${users.userid}&groupname=${showgroups.groupname }"> » 退出小组</a></span>';
				$("#gexit").html(gexit);
				var write='<a class="btn btn-primary btn-sm" role="button" href="/group/211/thread/create">发话题</a>';
				$("#write").html(write);
			}else{
				listStr+=gdata.groupname+'<a id="add-btn" class="btn btn-success btn-sm mlm" href="javaScript:void(0);">加入小组</a>';
				$("#gname").html(listStr);
				var status='<a id="add-btn" class="btn btn-info btn-sm pull-right" href="groups/joingroup?groupMember=${users.userid}&groupname=${showgroups.groupname }">加入小组</a>';
				$("#status").html(status);
			}
		});
